...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['amalfi-coast', 'sardinia', 'sicily'],
    'coast': ['amalfi-coast', 'cinque-terre', 'puglia'],
    'island': ['sicily', 'sardinia', 'capri'],
    'art': ['florence', 'rome', 'venice'],
    'culture': ['rome', 'florence', 'naples'],
    'history': ['rome', 'pompeii', 'florence'],
    'food': ['bologna', 'naples', 'rome'],
    'wine': ['tuscany', 'piedmont', 'puglia'],
    'relax': ['lake-como', 'amalfi-coast', 'sardinia'],
    'adventure': ['dolomites', 'cinque-terre', 'sicily'],
    'family': ['rome', 'florence', 'venice'],
    'honeymoon': ['amalfi-coast', 'venice', 'lake-como'],
    'romantic': ['venice', 'amalfi-coast', 'florence'],
    'budget': ['naples', 'bologna', 'puglia'],
    'luxury': ['amalfi-coast', 'lake-como', 'venice'],
    'northern-italy': ['milan', 'venice', 'lake-como'],
    'central-italy': ['rome', 'florence', 'tuscany'],
    'southern-italy': ['naples', 'amalfi-coast', 'puglia'],
    'islands': ['sicily', 'sardinia', 'capri'],
    'ski': ['dolomites', 'courmayeur', 'cortina'],
    'hiking': ['dolomites', 'cinque-terre', 'tuscany'],
  };
...
function buildSystemPrompt(): string {
  return `You are the Discovering Italy AI Trip Planner. You generate detailed day-by-day itineraries for Italy travel.
...
- Be specific: real place names, real prices, real transport options (Trenitalia, Italo, ferries, buses)
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-rome"
- Transport between cities should reference real train routes (e.g. Frecciarossa Rome-Florence), ferry routes, or bus services
...
- 15+ days: 2-3 items per day, brief descriptions (under 15 words each). Group similar days (e.g. "Days 5-6: Exploring Tuscany's hill towns"). Only include key activities, one meal, and accommodation.
...
{
  "title": "string — trip title like '7-Day Northern Italy Grand Tour'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "eur": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Rome'",
      "destination": "string — slug like 'rome'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Colosseum', 'Ponte Vecchio', 'Piazza San Marco'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Italy trip:\n${parts.join('\n')}`;
